...
class NotDeliveredOrdersFragment : androidx.fragment.app.Fragment() {

    private val disposables = CompositeDisposable()

    private val orderAdapter = OrderAdapter {
        parentFragmentManager.beginTransaction().apply {
            replace(R.id.fragment_container, NotDeliveredOrdersDetailsFragment.newInstance(it))
            addToBackStack(null)
            commit()
        }
    }

    private val notDeliveredOrdersQuery by lazy {
        DispergoDatabase.notDeliveredOrdersView(context!!).createQuery()
    }

    private val notDeliveredOrderLiveQuery by lazy {
        notDeliveredOrdersQuery.toLiveQuery()
    }

    private val factory by lazy {
        EntityDataSourceFactory(notDeliveredOrdersQuery::run, OrderMapper())
    }

    private val notDeliveredOrders by lazy {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        disposables.add(Search.query.subscribe({
            notDeliveredOrderLiveQuery.apply {
        ...
        notDeliveredOrderLiveQuery.apply {
            addChangeListener { event ->
                if (event.source == this) {
                    ...
                    notDeliveredOrders.value?.dataSource?.invalidate()
                }
            }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        notDeliveredOrders.observe(viewLifecycleOwner, Observer { orderAdapter.submitList(it) })
        (activity as MainActivity).supportActionBar?.title = getString(R.string.not_delivered)
