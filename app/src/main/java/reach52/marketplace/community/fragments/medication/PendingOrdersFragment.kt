...
class PendingOrdersFragment : androidx.fragment.app.Fragment() {
    private val disposables = CompositeDisposable()

    private val orderAdapter = OrderAdapter {
        parentFragmentManager.beginTransaction().apply {
            replace(R.id.fragment_container, PendingOrderDetailsFragment.newInstance(it))
            addToBackStack(null)
            commit()
        }
    }

    private val pendingOrdersQuery by lazy {
        DispergoDatabase.pendingOrdersView(context!!).createQuery()
    }

    private val pendingOrderLiveQuery by lazy {
        pendingOrdersQuery.toLiveQuery()
    }

    private val factory by lazy {
        EntityDataSourceFactory(pendingOrdersQuery::run, OrderMapper())
    }

    private val pendingOrders by lazy {
        ...
        pendingOrderLiveQuery.apply {
            addChangeListener { event ->
                if (event.source == this) {
                    ...
                    pendingOrders.value?.dataSource?.invalidate()
                }
            }
    ...
    override fun onCreateView(
        ...
        pendingOrders.observe(viewLifecycleOwner, Observer { orderAdapter.submitList(it) })
        (activity as MainActivity).supportActionBar?.title = getString(R.string.pending_order)

        return inflater.inflate(R.layout.fragment_pending, container, false).apply {
            pendingOrderRecyclerView.apply {
