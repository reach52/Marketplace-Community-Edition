...
class PickupFragment : androidx.fragment.app.Fragment() {
    private val disposables = CompositeDisposable()

    private val orderAdapter = OrderAdapter {
        parentFragmentManager.beginTransaction().apply {
            replace(R.id.fragment_container, PickedUpOrderDetailsFragment.newInstance(it))
            addToBackStack(null)
            commit()
        }
    }

    private val pickupOrdersQuery by lazy {
        DispergoDatabase.pickupOrdersView(context!!).createQuery()
    }

    private val pickupOrderLiveQuery by lazy {
        pickupOrdersQuery.toLiveQuery()
    }

    private val factory by lazy {
        EntityDataSourceFactory(pickupOrdersQuery::run, OrderMapper())
    }

    private val pickupOrders by lazy {
        ...
        pickupOrderLiveQuery.apply {
            addChangeListener { event ->
                if (event.source == this) {
                    ...
                    pickupOrders.value?.dataSource?.invalidate()
                }
            }
    ...
    override fun onCreateView(
        ...
        pickupOrders.observe(viewLifecycleOwner, Observer { orderAdapter.submitList(it) })
        (activity as MainActivity).supportActionBar?.title = getString(R.string.received_order)
