...
import kotlinx.android.synthetic.main.item_followup.view.*
import org.threeten.bp.ZoneOffset
import org.threeten.bp.ZonedDateTime
import org.threeten.bp.format.DateTimeFormatter

class FollowUpAdapter (private val detailClicked: (String) -> Unit) :
        ListAdapter<Entity<FollowUp>, FollowUpAdapter.ViewHolder>(diffCallBack()){

    companion object {
        fun diffCallBack() = object : DiffUtil.ItemCallback<Entity<FollowUp>>() {
            override fun areItemsTheSame(oldItem: Entity<FollowUp>, newItem: Entity<FollowUp>): Boolean = oldItem.id == newItem.id
            override fun areContentsTheSame(oldItem: Entity<FollowUp>, newItem: Entity<FollowUp>): Boolean = oldItem.record == newItem.record
        }
    }
    ...
    class ViewHolder(private val view: View) : RecyclerView.ViewHolder(view) {
        fun display(followUp: FollowUp) {
            ...
            val colorValue = when {
                dateFollowUp == dateNow -> {"#FFBF00"} // amber
                dateFollowUp == datePlus -> {"#008000"} // green
                followUp.dateFollowUp.isBefore(ZonedDateTime.now(ZoneOffset.UTC).plusHours(8)
                ) -> {"#C82333"} // red
                else -> {"#0000FF"} // blue
            }
