...
class ConsumerHealthOrderAdapter(private val orderClicked: (String) -> Unit) :
        PagedListAdapter<Entity<ConsumerHealthOrder>, ConsumerHealthOrderAdapter.ViewHolder>(diffCallback()){
    companion object{
        fun diffCallback() = object : DiffUtil.ItemCallback<Entity<ConsumerHealthOrder>>() {
            override fun areItemsTheSame(a: Entity<ConsumerHealthOrder>, b: Entity<ConsumerHealthOrder>):
                    Boolean = a.id == b.id
            override fun areContentsTheSame(a: Entity<ConsumerHealthOrder>, b: Entity<ConsumerHealthOrder>): Boolean =
                    a.record == b.record
        }
    }
...
//        fun setData(order: ConsumerHealthOrder) {
//            view.orderStatusLabel.setBackgroundColor(statusColor(order.currentStatus.status))
//            view.orderNumberItem.text = order.customer.firstName.plus(" ").plus(order.customer.middleName).plus(" ").plus(order.customer.lastName)
//            view.orderResidentAddress.text = order.deliveryAddress
//            view.medicineCount.text = order.orderLines.list.size.toString()
//        }
    }
}
